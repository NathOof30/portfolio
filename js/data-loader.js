    ...
    loadProjectsPage() {
        ...
        pageProjets.innerHTML = this.data.projects.categories.map(category => {
            const projectsHTML = category.projects.map(project => {
                ...
                if (project.imageFullQuality || project.image) {
                    allMedia.push(`image:${project.imageFullQuality || project.image}`);
                }
                
                // Ajouter les images supplémentaires
                if (project.additionalImages && project.additionalImages.length > 0) {
                    project.additionalImages.forEach(img => {
                        allMedia.push(`image:${img}`);
                    });
                }
                ...
                const dataMedia = allMedia.length > 0
                    ? `data-media="${allMedia.join(',')}"`
                    : '';
                ...
                return `
                    <article class="carte-projet" id="projet-${project.id}" ${dataMedia}>
